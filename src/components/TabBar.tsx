...
export const TabBar: React.FC<TabBarProps> = ({ className }) => {
    const { tabs, activeId, activate, close, closeOthers, closeAll, reorderTabs } = useTabsStore()
    ...
    return (
        <div className={cn('flex items-center border-b bg-background overflow-x-auto', className)}>
            {tabs.map((tab, index) => (
                <div
                    key={tab.id}
                    className={cn(
